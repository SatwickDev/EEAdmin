...
class ChromaDBRepositoryManager:
    ...
    Now supports:
    - Environment variable configuration for ChromaDB host/port
    - Per-customer ChromaDB client management
    - Integration with app.utils.chroma_manager for multi-tenant support
    """
    
    def __init__(self, db_client, chroma_host=None, chroma_port=None):
        self.db = db_client
        ...
        self._default_chroma_host = chroma_host or os.getenv('CHROMADB_HOST', 'localhost')
        self._default_chroma_port = chroma_port or int(os.getenv('CHROMADB_PORT', 8000))
        
        # Do not create a global Chroma client at initialization
        # Use per-customer manager at runtime via _get_client_for_customer()
        self.chroma_client = None
        
        logger.info(f"ChromaDB Repository Manager initialized (default: {self._default_chroma_host}:{self._default_chroma_port})")
    ...
    def _get_client_for_customer(self, customer_id: Optional[str] = None) -> Optional[chromadb.HttpClient]:
        """Get ChromaDB client for a specific customer or use default
        
        Preference order:
        1. Use self.chroma_client if already initialized (for backward compatibility)
        2. Get per-customer client from app.utils.chroma_manager
        3. Fall back to default host/port configuration
        
        Args:
            customer_id: Optional customer ID. If None, attempts to infer from Flask request
            
        Returns:
            ChromaDB HttpClient or None if unavailable
        """
        # If a global client was set externally, use it
        if self.chroma_client:
            return self.chroma_client
        
        # Try to get per-customer client from chroma_manager
        try:
            from app.utils.chroma_manager import get_chroma_client_for_customer, get_request_customer_id
            
            # If no customer_id provided, try to get from Flask request
            ...
            # Get database client for chroma_manager
            try:
                from app import db as app_db
            except Exception:
                app_db = None
            
            # Get customer-specific client
            client = get_chroma_client_for_customer(customer_id, app_db)
            if client:
                return client
                
        except Exception as e:
            logger.debug(f"Could not get per-customer ChromaDB client: {e}")
        
        # Fall back to creating a default client with environment variables
        try:
            os.environ["ANONYMIZED_TELEMETRY"] = "False"
            client = chromadb.HttpClient(
                host=self._default_chroma_host,
                port=self._default_chroma_port,
                settings=Settings(anonymized_telemetry=False)
            )
            logger.debug(f"Using default ChromaDB client at {self._default_chroma_host}:{self._default_chroma_port}")
            return client
        except Exception as e:
    ...
    def get_chroma_collection_info(self, collection_name: str, customer_id: Optional[str] = None) -> Dict[str, Any]:
        """Get information about a ChromaDB collection
        
        Args:
            collection_name: Name of the ChromaDB collection
            customer_id: Optional customer ID for per-customer client
            
        Returns:
            Dictionary with collection information
        """
        client = self._get_client_for_customer(customer_id)
        if not client:
    ...
    def search_in_collection(self, collection_name: str, query: str, n_results: int = 5, customer_id: Optional[str] = None) -> List[Dict]:
    ...
    def get_collection_sample_documents(self, collection_name: str, limit: int = 5, customer_id: Optional[str] = None) -> List[Dict]:
        ...
        Returns:
            List of formatted sample documents
        """
        client = self._get_client_for_customer(customer_id)
        if not client:
            return []
