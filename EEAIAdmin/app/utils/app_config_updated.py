...
try:
    logger.info("Starting Azure OpenAI configuration using api_config_manager...")
    ...
    if not is_azure_openai_enabled():
        ...
        logger.info("OpenAI configured successfully using api_config_manager.")
        
except Exception as e:
    logger.error(f"Error configuring OpenAI: {e}")
    logger.error(f"Environment variables check - AZURE_OPENAI_ENABLED: {os.getenv('AZURE_OPENAI_ENABLED', 'true')}")
...
AZURE_OPENAI_API_VERSION = get_azure_api_version()
...
# SQLAlchemy Engine (deprecated - using MongoDB)
engine = None
logger.info("SQLAlchemy engine disabled - application uses MongoDB")
