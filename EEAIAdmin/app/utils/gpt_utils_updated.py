...
def load_extraction_fields(product_name, document_type):
    ...
    Args:
        product_name (str): Product name (e.g., "EE", "CE")
        document_type (str): Document type (e.g., "register_import_lc", "invoice")
        
    Returns:
        dict: Field descriptions for the specified product and document type
    """
    ...
    if not os.path.exists(config_path):
        logger.warning(f"function_fields.json not found at {config_path}")
        return {}
    ...
    try:
        ...
        logger.error(f"Error loading extraction fields: {e}")
        return {}
