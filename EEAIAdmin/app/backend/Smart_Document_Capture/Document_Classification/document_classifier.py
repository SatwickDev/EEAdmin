...
class DocumentClassifier:
    def __init__(self):
        ...
        # Document_Classification -> AI_Document_Processor -> backend -> app -> EEAIAdmin (project root)
        base_dir = Path(__file__).parent.parent  # AI_Document_Processor
        project_root = base_dir.parent.parent.parent  # Go up to EEAIAdmin (project root)
    ...
    def reload_from_disk(self) -> bool:
        ...
        try:
            ...
            logging.info(
                "DocumentClassifier.reload_from_disk: reloaded -> entity_mappings=%d, entity_descriptions=%d, document_categories=%d",
    ...
    def _load_document_categories(self) -> Dict:
        ...
        try:
            import yaml
            # Path: document_classifier.py -> Document_Classification -> Smart_Document_Capture -> backend -> app -> EEAIAdmin
            ...
            with open(str(config_path), 'r', encoding='utf-8') as f:
                ...
                logging.info(f"Successfully loaded prompt_config with keys: {list(config.keys())}")
    ...
    # NOTE: _load_document_fields removed - DOC_LIST is deprecated, use entity_mappings instead
